...
//!
//! Both drive the *built* `satysfi-rust` binary as a subprocess (the
...
fn envelopes_use_of_document_compiles_to_pdf() {
    ...
    let output = Command::new(bin())
        ...
        .arg("--no-cache")
        .output()
        .expect("failed to run the satysfi-rust binary");
...
fn deps_with_v006_errors_naming_both_axes() {
    ...
    let deps = dir.join("satysfi-deps.yaml");
