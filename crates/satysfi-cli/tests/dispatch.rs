...
//! materialised layout matches `satysfi_loader`'s `@require:` contract.
...
#[cfg(unix)]
#[test]
fn argv0_satysfi_is_compile_personality() {
    let out = run_as("satysfi", &["--help"]);
    assert!(out.status.success(), "satysfi --help should succeed");
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("Compile a SATySFi"),
        "satysfi --help should describe the compiler:\n{stdout}"
    );
...
#[cfg(unix)]
#[test]
fn satysfi_rust_satyrographos_subcommand_form() {
    // The same package-manager entry point, reached as the nested subcommand
    // under the `satysfi-rust` personality.
...
#[cfg(unix)]
#[test]
fn bare_satysfi_rust_without_input_is_usage_error() {
    let out = run_as("satysfi-rust", &[]);
...
fn multicall_install_creates_working_aliases() {
    ...
    let satysfi_alias = dir.join("satysfi");
    let satyro_alias = dir.join("satyrographos");
    assert!(satysfi_alias.exists(), "satysfi alias created");
...
fn install_then_loader_resolves_the_package() {
    ...
    let root = work.join("root");
    satysfi_satyrographos::install(
        &src,
        &satysfi_satyrographos::InstallOptions {
