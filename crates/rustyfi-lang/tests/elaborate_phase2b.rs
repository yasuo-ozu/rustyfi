...
use rustyfi_lang::ast::{Ast, IText};
use rustyfi_lang::value::Value;
use rustyfi_lang::{elaborate, eval, primitives};
...
fn qualified_math_command_resolves_to_the_mangled_key() {
    ...
    let rustyfi_lang::ast::Ast::LetIn(qualified_name, _alias_value, body) = *rest else {
        panic!("expected LetIn (the qualified alias) nested inside, got {rest:?}");
    };
    assert_eq!(qualified_name, "M.\\cmd");
    let rustyfi_lang::ast::Ast::MathText(elems) = *body else {
        panic!("expected a MathText literal, got {body:?}");
    };
    assert_eq!(elems.len(), 1);
    match &elems[0] {
        rustyfi_lang::ast::MathElem::Cmd { name, .. } => {
