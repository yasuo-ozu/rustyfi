...
//! `satysfi-pdf/src/lib.rs`/`cid.rs`), driven through the public
...
//! `crates/satysfi-cli/tests/fixtures/v01-strings.saty` is the end-to-end
//! version through the real `register-document-information` primitive).

use satysfi_backend::{DocExtras, DocInfo, Length, Page, PageGeometry};
...
fn render_pdf_with_emits_info_dict_when_registered() {
    ...
    let bytes =
        satysfi_pdf::render_pdf_with(&geometry, &pages, &[], &extras).expect("render must succeed");
...
fn render_pdf_with_omits_info_dict_when_unregistered() {
    ...
    let via_default = satysfi_pdf::render_pdf(&geometry, &pages, &[]).expect("render must succeed");
    let via_with = satysfi_pdf::render_pdf_with(&geometry, &pages, &[], &DocExtras::default())
