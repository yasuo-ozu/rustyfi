...
// Shared Image XObject support. `render_pdf` (base-14, below) and
// `render_pdf_ttf` (CID-keyed TrueType, `cid.rs`) are otherwise separate
// writers, but an `Image` box is rendered *identically* by both, so that path
// lives here once and `cid.rs` imports it.

/// Every `ImageId` (raw `usize` index into a `DocumentValue::images`-shaped
/// table) that appears in at least one placed line across `pages`, OR in a
/// page's `overlays` deco-graphics underlay (`fire_hooks`'
/// `page_graphics`) — only an image actually placed on a page gets an
/// XObject, not one merely decoded.
///
/// Scanning the overlay is essential for images drawn *inside a decoration*
/// (`figbox`'s `+fig-on-right` draws its figure with `draw-text` from the
/// frame's `deco`, so the box only ever lives in `page_graphics`): each
...
/// the page's `/Resources /XObject` entry and the content stream's `Do`
/// operand, which must agree.
...
/// dictionaries.
///
/// **JPEG DCTDecode passthrough.** When `im.jpeg_dct` is `Some` (a
/// baseline/extended-sequential 8-bit JPEG, per
/// `ImageResource::sniff_baseline_jpeg_dct`), the ORIGINAL, still-DCT-encoded
/// file bytes are embedded verbatim with `/Filter /DCTDecode` and a
/// `/ColorSpace` from the JPEG's own component count (`/DeviceGray` for 1,
/// `/DeviceRGB` for 3), matching upstream's own JPEG special-case. Every
/// other image is flat, uncompressed 8-bit `DeviceRGB` samples with no
/// `/Filter` at all.
fn write_image_xobjects(
    ...
    for &id in used {
        let Some(im) = images.get(id) else {
            // An id past the end of the image table should not happen, but a
            // page silently missing one image beats a panic.
            continue;
        };
        if im.pdf.is_some() {
...
// writers like the raster support above.
...
/// collide in the shared `/XObject` dictionary, which does not itself
/// distinguish the two (the `/Subtype` lives inside each stream).
fn form_res_name(id: usize) -> String {
    format!("Fm{id}")
}

/// Re-emit one neutral `ObjRepr` value into a fresh `Obj` writer, remapping
/// any `Ref(local_id)` through `remap`; an unresolved local id degrades to
/// `Null` rather than panicking. A `Stream` payload cannot legally appear
/// here (streams must be indirect objects in PDF; the importer only produces
/// one at the top level of an indirect entry, reached via `write_pdf_obj`) —
/// written as `Null` if it somehow does.
fn write_pdf_obj_value(obj: pdf_writer::Obj<'_>, repr: &ObjRepr, remap: &BTreeMap<u32, Ref>) {
    match repr {
        ...
        // `pdf-writer` only implements `Primitive` for `i32`; integers in a
        // resource subtree fit comfortably.
...
/// Write one imported object (the `collect_direct_objects` / `Pdf.addobj`
/// analogue) at `out_ref`: a `Stream` becomes an indirect stream (dict
/// entries copied verbatim, minus `/Length` which `pdf-writer` derives),
/// anything else a plain indirect object.
...
/// `pdf` payload (`load-pdf-image`); ids without one were already handled by
/// `write_image_xobjects`.
///
/// Per id: allocate a fresh output `Ref` for every non-zero local id in
/// `PdfPageResource.resources`, write each of those objects with its
/// references remapped, then emit the page's own content stream as a
/// `/Subtype /Form` XObject whose `/BBox` is the source `/MediaBox` and whose
/// `/Resources` is the (also remapped) local-id-0 root dictionary — upstream
/// `loadPdf.ml`'s `xobject_of_page`.
fn write_form_xobjects(
    ...
    for &id in used {
        ...
        let (x0, y0, x1, y1) = pdf_res.media_box;
        {
            ...
            // Identity `/Matrix`: form space == the source page's own
            // MediaBox space. The box-to-page scale/translate lives entirely
            // in the placement `cm` operator (`place_form`).
...
/// y-coordinate a text run on this line uses for its `Td`/`next_line`,
/// because an image XObject's unit square is placed with its *bottom-left*
/// corner at the `cm` matrix's translation and `PureHorzBox::Image` sits
/// entirely above the baseline (all height, zero depth, per `linebreak.rs`'s
/// `layout_line`): the baseline *is* the image's bottom edge.
...
/// box: `q  sx 0 0 sy (tx - sx*x0) (ty - sy*y0) cm  /FmN Do  Q`.
///
/// Unlike an Image XObject (unit square, so `place_image`'s matrix is a plain
/// `[w, 0, 0, h, tx, ty]` scale), a Form XObject draws in its own user space
/// — the source page's `/MediaBox` coordinates — so the CTM must map that box
/// onto the placed `(tx, ty, w, h)` box: scale by `w/(x1-x0)`, `h/(y1-y0)`
/// and translate the MediaBox's own origin `(x0, y0)` to `(tx, ty)`. `/BBox`
/// clips in the form's own unscaled user space, so this CTM is the only place
/// the box-to-page scale factor is applied.
fn place_form(
    content: &mut Content,
    id: usize,
    tx: f32,
...
/// Tj / ET` group per glyph: each glyph carries its own font/size
/// (`glyph.info`) and offset (`glyph.dx`/`dy`) relative to the box's placed
/// anchor `(anchor_x, anchor_y)`, the same already-flipped `(line.x + dx,
/// paper_h - baseline_y)` a text run's `Td` uses. `glyph.dy > 0` raises it (a
...
/// `glyph.text` for `render_pdf`, a glyph-id run for `render_pdf_ttf`
/// (`cid.rs`) that additionally special-cases `glyph.gid.is_some()` (a
/// raw MATH-table variant glyph, emitted directly rather than re-derived from
/// `text`) — so the whole glyph, not just `info`/`text`, is threaded through.
pub(crate) fn place_math(
    ...
    for glyph in glyphs {
        ...
        // Non-black only, matching the `InnerString` arms' `q…Q` guard.
...
/// anchor `(tx, ty)`, with the *text* emission (the one thing that differs
/// between the two writers) threaded through as the `emit_line` callback.
///
/// **Top-aligned stand-in.** `place_block_at` (rustyfi-backend) lays `block`
/// out from a fixed `(0, 0)` page-y-down origin; this shifts that whole stack
/// so the FIRST content line's baseline sits exactly at the anchor `ty`, with
/// every later line falling further down the page (subtracted from `ty`,
/// since PDF y is up). `embed-block-top`'s `adjust_to_first_line` (exact
/// upstream baseline alignment) is the faithful refinement.
pub(crate) fn place_embedded_block(
    ...
    // `ty`: the FIRST for `embed-block-top`, the LAST for
    // `embed-block-bottom` (upstream `adjust_to_first_line` /
    // `adjust_to_last_line`). Every other line is offset by the difference of
    // their placed baselines (larger `baseline_y` = lower = smaller PDF `y`).
...
pub(crate) fn write_annotations(
    ...
    for a in annots {
        if a.page >= n_pages {
            continue; // out-of-range page: skip gracefully
        }
        let r = next_ref();
...
/// Emit the PDF `/Info` dictionary at `id` from
/// `register-document-information`'s registered value.
/// `pdf.document_info(id)` self-registers with the file trailer (pdf-writer
/// `structure.rs`), so the caller only allocates `id` and calls this once,
/// gated on `extras.doc_info.is_some()` — an unregistered document emits no
/// `/Info` object at all. `/Title`/`/Subject`/`/Author` are written only when
/// `Some`; `/Keywords` only when non-empty, joined with a single space
/// (upstream `String.concat " "`, `documentInformationDictionary.ml`).
/// DOCUMENTED DEVIATION: `/Creator`/`/Producer` are written unconditionally
/// *once this function runs* (i.e. only when the dict is registered at all) —
/// upstream emits them on EVERY document, its `/Info` dict always existing.
...
/// Same as [`render_pdf`], but also emits the extras (`/Annots`,
/// `/Dests`, `/Outlines`, per-page deco-graphics underlays) accumulated
/// while evaluating the document (`DocumentValue::extras`).
pub fn render_pdf_with(
    ...
    // Annotations, named destinations, outline.
        ...
        pages.iter().zip(&page_ids).zip(&content_ids).enumerate()
    {
        ...
        // three base fonts above make: an unused resource entry is legal.
...
/// coordinates (downward) to PDF (upward). `overlay` (deco graphics,
/// already in absolute PDF y-up page coordinates — see `fire_hooks`) is
/// drawn FIRST, so it sits under the page's text/images. `images` is only
/// consulted to tell an ordinary raster `Image` box from an imported-PDF-page
/// one (`place_image` vs `place_form`).
fn page_content(
    page: &Page,
    paper_h: f32,
    ...
    // for an empty slice — guard it so an overlay-free page emits nothing.
    if !overlay.is_empty() {
...
/// already happened in the caller's `ty`). **Reentrant**: a `Tabular` box's
/// cells hold their own already-laid-out `PureHorzBox` runs, emitted through
/// this same path recursively.
///
...
/// (`tab.rules`) go through `place_graphics`, whose own `cm` translate to
/// `(tx, ty)` puts its box-local y-up path coordinates in that same frame.
fn emit_box(
    content: &mut Content,
    bx: &PureHorzBox,
    tx: f32,
    ...
    match bx {
        ...
        PureHorzBox::Math { glyphs, rules, .. } => {
            ...
            // `math_vertical_variant`), so this always encodes `g.text`.
            ...
            // The fraction bar/radical sign+overbar are `Fill`s, not
            // glyphs — placed through `place_graphics` at the SAME
            // already-flipped anchor `place_math` just used for the glyphs
            // (no second y-flip belongs here).
        ...
        // An inline frame's contents, on the frame's own baseline. The
        // frame's deco graphics are NOT emitted here — they were fired
        // lang-side into `DocExtras::page_graphics` and drawn as the page
        // underlay (`page_content`'s `overlay` prologue).
        PureHorzBox::Frame { contents, .. } => {
...
/// Emit `elems` (already box-local — see `PureHorzBox::Graphics`) into
/// `content`, wrapped in one `save_state`/`transform`/`restore_state` that
/// translates the whole box to its placed PDF-space anchor `(tx, ty)` — the
/// **same** already-flipped `(line.x + dx, paper_h - baseline_y)` anchor a
/// text run on that line uses, so element coordinates stay box-local (exactly
/// `graphicD.ml`'s per-box `cm` wrapping).
///
...
/// *inside* the `q; cm` translate below, so PDF text ops (`BT`/`Td`/`Tj`)
/// compose with the CTM the same way a filled path does. Adding an absolute
/// anchor or a second y-flip here would double-place the run — don't.
pub(crate) fn place_graphics(
    ...
    for elem in elems {
        ...
        match elem {
            ...
            // `draw-text`: re-enter the writer's own per-box emission at
            // box-local coordinates `pt + dx` — see "Text and the CTM" above.
            ...
            // 0.1's `graphics` collection container nodes. Never reached
            // by any 0.0.6 program (no 0.0.6-visible prim constructs
            // `Group`/`Clip`).
            GraphicsElem::Group(inner) => {
                // Zero anchor: the outer q/cm(tx,ty) frame is already active,
                // and a nested translate of (0,0) is what upstream's flat
                // `List.concat` renders to.
...
/// `graphicD.ml`'s `pdfops_of_path`). Closings: `Open` emits nothing, `Line`
/// emits `close_path` (`h`), `Bezier(c1, c2)` a final `cubic_to(c1, c2,
/// start)` then `close_path`.
...
/// Encode to WinAnsi. Accepts ASCII 32..=126 (what the metrics tables cover);
/// anything else is an error rather than mojibake.
