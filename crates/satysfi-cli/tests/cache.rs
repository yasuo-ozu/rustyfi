//! Compile-cache behaviour, driven through the *built* `satysfi-rust` binary
...
//! cache never touches the developer's real `~/.cache/satysfi-rust` and no two
...
/// The built `satysfi-rust` binary (cargo hands this to the integration tests
/// of the crate that defines the `[[bin]]`).
fn bin() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_satysfi-rust"))
}

/// This repo's `lib-satysfi/`, resolved from the crate manifest dir exactly as
...
/// `lib-satysfi/` and using `cache_dir` for the compile cache. `extra` carries
/// flags like `--no-cache`.
...
/// Timing demonstration (run with `cargo test -p satysfi-cli --ignored`): a
