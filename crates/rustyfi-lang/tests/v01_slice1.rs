//! The semantic-equivalence integration test —
//! `compile_document_v1` proven end-to-end (pre-flip: `rustyfi-syntax`'s
...
//! for geometry equivalence — "same core IR, different surface, no semantic
//! drift" (see `assert_geometry_equivalent` below for what is compared).
...
/// (`main.rs`) — reproduced locally because there is no `rustyfi` library
...
/// The "same core IR, different surface" bar, expressed observably: equal
/// page geometry, per-page line counts, and placed-line x/baseline_y
/// sequences — deliberately NOT full `PlacedLine`/`Ast` equality (spans
/// differ between the two source files, and full-box-tree equality would
/// incidentally re-assert internal ids with no semantic weight).
