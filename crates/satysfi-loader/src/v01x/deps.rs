//! `satysfi-deps.yaml` decoder — Ld3b-1 (Axis B increment Ld3b, §3.1/§5.1 of
...
//! unit tests.
//!
//! `satysfi-deps.yaml` is what `saphe` *writes* before invoking `satysfi
...
//! `satysfi-saphe` crate (plan Sa1) exchanges this same file format and MUST
...
/// Decoded `satysfi-deps.yaml` — upstream `depsConfig.ml:32-37`
...
pub(crate) struct EnvelopeSpec {
    ...
    pub name: String,
    /// Absolute path to the envelope's own `satysfi-envelope.yaml`
...
/// Load + decode + validate a `satysfi-deps.yaml`. Errors:
...
mod tests {
    ...
    fn deps_demo_shaped_decodes_and_used_as_map() {
        ...
        assert_eq!(
            stdlib.path,
            PathBuf::from("/synthetic-root/store/stdlib.0.0.1/satysfi-envelope.yaml")
        );
...
",
    ...
    fn deps_duplicate_envelope_name_decodes_fine() {
        let f = TempFile::write(
            ...
            "envelopes:
- name: pkg
  path: \"/a/satysfi-envelope.yaml\"
  dependencies: []
  test_only: false
- name: pkg
  path: \"/b/satysfi-envelope.yaml\"
  dependencies: []
  test_only: false
dependencies: []
test_dependencies: []
",
    ...
    fn deps_file_missing_is_not_found() {
        let path = std::env::temp_dir().join("satysfi-loader-deps-test-does-not-exist.yaml");
