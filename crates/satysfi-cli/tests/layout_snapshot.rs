...
//! 3. **The post-page-break placed-box model** (`satysfi_backend::pagebreak
...
//!   not random.
//! - Base-14 metrics only (`satysfi_pdf::Base14Metrics`), so this test needs
...
//!
//! ```text
//! UPDATE_SNAPSHOTS=1 cargo test -p satysfi-cli --test layout_snapshot
//! ```
//!
...
use satysfi_backend::{Length, PureHorzBox, VertBox};
use satysfi_lang::value::DocumentValue;
use satysfi_loader::{LoadOptions, LoadedCst};
use satysfi_pdf::Base14Metrics;
...
/// `lib-satysfi/`, merge dependency preludes ahead of the entry's own
/// (exactly `satysfi-cli`'s private `merge_program`, reproduced the same way
/// `tests/e2e.rs`'s `load_and_merge` already does), and compile with
/// deterministic base-14 metrics — never touching `satysfi-pdf`'s writers.
...
/// `lib-satysfi/dist-v01/packages/`, exactly like `tests/xver_capstone.rs`
