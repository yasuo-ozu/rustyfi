...
//! `satysfi-rust` binary (like `tests/cache.rs`/`tests/dispatch.rs`), not the
...
//! — none of which exist when calling `satysfi_lang`/`satysfi_pdf` directly.
//!
...
//! `crates/satysfi-pdf/tests/ttf.rs`. They also skip when `pdftotext` is
//! unavailable.
...
/// The built `satysfi-rust` binary (cargo provides this env var to the
...
/// `crates/satysfi-pdf/tests/ttf.rs`'s `find_regular_font`.
...
/// fixture's source text is static (unlike `satysfi-pdf/tests/ttf.rs`, which
...
fn font_supports_fixture_glyphs(font_path: &Path) -> bool {
    ...
    let Ok(store) = satysfi_pdf::TtfFontStore::load(font_path, None, None) else {
        return false;
    };
...
/// `satysfi_pdf::fonts`'s module docs), naming `font_path` as the sole
...
/// built in `cid.rs` works end to end, not just in `satysfi-pdf`'s own unit
/// tests).
#[test]
...
/// anywhere (no flags, no `$SATYSFI_FONT_DIR`, no `dist/hash/` under
...
fn no_font_config_matches_base14_byte_for_byte() {
    ...
    let via_lib = satysfi_pdf::render_pdf(&doc.geometry, &doc.pages).expect("render fixture");
...
fn changing_font_config_invalidates_the_compile_cache() {
    ...
    let second = Command::new(bin())
        ...
        .args(["--font-dir".as_ref(), font_root.as_os_str()])
        .output()
        .expect("spawn satysfi-rust (second)");
    ...
    let third = Command::new(bin())
        ...
        .args(["--font-dir".as_ref(), font_root.as_os_str()])
        .output()
        .expect("spawn satysfi-rust (third)");
...
/// `satysfi_pdf::fonts::FontRegistry::discover`.
#[test]
