...
//! `stdlib` package modules — `lib-satysfi/dist-v01/packages/{color,basic,
...
//! the real production loader (`satysfi_loader::load`, `lib_root =
//! dist-v01/packages`, `SatysfiVersion::V0_1`) — not merely parsed.
//!
...
//!    REAL-DOCUMENT bar: `satysfi_loader::load` + `satysfi_lang::
...
/// This repo's `lib-satysfi/dist-v01/packages/` — Wave 0's real home —
...
impl TempDoc {
    fn new(tag: &str, src: &str) -> TempDoc {
        ...
        let path = std::env::temp_dir().join(format!(
            "satysfi-lang-v01-stdlib-{tag}-{}-{}.saty",
            std::process::id(),
            n
        ));
...
>";
        ...
        let program = satysfi_loader::load(&doc.0, &opts).expect("v01-mini + color should load");
        assert_eq!(
            program.files.len(),
            3,
        ...
        let doc_value = satysfi_lang::compile_document_v1(&program.files, &Mono)
