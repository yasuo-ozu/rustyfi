...
//! injection, §4) together with `satysfi_backend::break_into_lines`,
//! proving end to end:
//!
...
use satysfi_lang::hyphenation::hyphenate_word;
use satysfi_lang::primitives;
use satysfi_lang::value::Env;

/// Every char is half an em wide (mirrors `satysfi-backend/tests/
...
fn a_huge_hyphen_penalty_disables_the_break_even_though_the_line_overflows() {
    ...
    // `satysfi-backend/tests/linebreak.rs`'s
