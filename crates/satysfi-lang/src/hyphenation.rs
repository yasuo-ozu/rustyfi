...
//! (in `satysfi-backend`) only stores a lightweight `HyphenLang` tag (D3) —
...
use satysfi_backend::HyphenLang;
use std::collections::HashMap;
use std::io::Cursor;
use std::sync::{Arc, Mutex, OnceLock};

/// Map the port's crate-independent tag (`satysfi_backend::HyphenLang`) to
...
/// no-filesystem property). This keeps `crates/satysfi-lang/Cargo.toml`'s
