...
fn cid_colored_run_is_scoped_in_q_rg_q_black_run_emits_no_color_op() {
    ...
    let store = rustyfi_pdf::TtfFontStore::load(&path, None, None).expect("load font");
    let geo = geometry();
    let font = FontKey(0);
    ...
    let bytes = rustyfi_pdf::render_pdf_ttf(&geo, &[page], &store, &[]).expect("render");
    ...
    if let Ok(which) = Command::new("which").arg("pdftotext").output() {
        if which.status.success() {
            ...
            tmp.push(format!("rustyfi-pdf-text-color-cid-{}.pdf", std::process::id()));
