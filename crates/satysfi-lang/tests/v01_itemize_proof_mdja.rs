...
//! production loader (`satysfi_loader::load`, `lib_root =
...
impl TempDoc {
    fn new(tag: &str, src: &str) -> TempDoc {
        ...
        let path = std::env::temp_dir().join(format!(
            "satysfi-lang-v01-itemize-proof-mdja-{tag}-{}-{}.saty",
            std::process::id(),
            n
        ));
...
// unidata capstone tests AND `crates/satysfi-cli/tests/e2e.rs`'s
