...
//! (`crates/satysfi-lang/tests/images.rs` covers that, and
//! `crates/satysfi-pdf/tests/image.rs` covers the full round trip through
//! the PDF writer's `/Filter /DCTDecode` embedding).

use std::path::Path;

use satysfi_backend::ImageResource;

/// The checked-in fixture: a tiny 8x4 baseline (SOF0), 8-bit, 3-component
/// (YCbCr/RGB) JPEG generated with Pillow — same bytes as
/// `satysfi-pdf/tests/fixtures/dot.jpg` / `satysfi-lang/tests/fixtures/dot.jpg`
/// (duplicated per this workspace's existing per-crate fixture convention;
/// see `satysfi-lang/tests/images.rs`'s `fixture_path` doc comment for why).
...
fn a_png_is_not_mistaken_for_a_jpeg() {
    let png = std::fs::read(
        Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap()
            .join("satysfi-pdf/tests/fixtures/dot.png"),
    )
