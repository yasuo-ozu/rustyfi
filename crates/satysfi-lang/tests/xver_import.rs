...
use satysfi_backend::{FontKey, FontMetrics, GraphicsElem, Length};
...
fn load_xver_pagebreak_fixture(tag: &str, entry_tail: &str) -> Vec<satysfi_loader::LoadedFile> {
    ...
    let program = satysfi_loader::load(&dir.path().join("entry.saty"), &opts).unwrap_or_else(|e| {
        panic!("loading the X2a page-break xver fixture ({tag}) should succeed: {e}")
    });
    assert!(
        program.files.iter().any(|f| matches!(f.cst, satysfi_loader::LoadedCst::V0_0_6(_))),
...
fn xver_boundary_deco_export_curried_prefix_still_rejected() {
    ...
    let mono = Mono;
    let err = satysfi_lang::compile_document_v1(&program.files, &mono).expect_err(
