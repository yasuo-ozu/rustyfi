...
//! (no `satysfi-lang` parse/eval needed to exercise the writer in
//! isolation); `crates/satysfi-lang/tests/pdf_images.rs` covers the
...
/// walk (`convert_pdf_object`, `satysfi-lang/src/primitives.rs`) would
...
fn text_only_document_emits_no_form_xobject() {
    ...
    let geometry = geometry();
    let bytes = satysfi_pdf::render_pdf(&geometry, std::slice::from_ref(&page), &[]).unwrap();
    let text = String::from_utf8_lossy(&bytes);
