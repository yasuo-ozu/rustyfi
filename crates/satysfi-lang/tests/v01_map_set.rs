...
//! `lib-satysfi/dist-v01/packages/{map,set}.satyg` — transliterated from
//! the real upstream source (`saphe-split@b836d512:lib-satysfi/packages/
...
//! exactly what `satysfi_lang::compile_document_v1`'s real pipeline builds
...
//! (`satysfi_loader::load`, `lib_root = dist-v01/packages`) via a
...
use satysfi_syntax::{parse_file_v1, SatysfiVersion};

/// `lib-satysfi/dist-v01/packages/` — same resolution as `v01_stdlib.rs`'s/
...
fn lib_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../lib-satysfi/dist-v01/packages")
}
...
impl TempDoc {
    fn new(tag: &str, src: &str) -> TempDoc {
        ...
        let path = std::env::temp_dir().join(format!(
            "satysfi-lang-v01-map-set-{tag}-{}-{}.saty",
            std::process::id(),
            n
        ));
...
fn load_real_deps(tag: &str, requires: &str) -> Vec<LoadedFile> {
    ...
    let mut program = satysfi_loader::load(&doc.0, &opts)
...
fn compile_with_harness(
    ...
    satysfi_lang::compile_document_v1(&files, &Mono)
}
