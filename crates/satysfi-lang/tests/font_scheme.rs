...
use satysfi_backend::{Context, FontKey, FontMetrics, Length, PureHorzBox, Script};
use satysfi_lang::ast::IText;
...
fn mixed_script_paragraph_produces_two_font_keys_and_scaled_size() {
    ...
    ctx.font_scheme[Script::Kana as usize] = satysfi_backend::ScriptFont {
        font: FontKey(7),
        ratio: 0.88,
        rising: 0.0,
    };
    ...
    for hb in &boxes {
        let satysfi_backend::HorzBox::Pure(PureHorzBox::InnerString { info, .. }) = hb else {
            continue;
        };
