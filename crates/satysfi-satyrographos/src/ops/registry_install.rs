...
pub fn install_registry(
    ...
    let report = install_resolved(name, &resolved, opts)?;
    Ok((report, resolved))
}
...
pub fn install_resolved(
    ...
    // Step 3: fetch, then verify BEFORE touching dist/ — a mismatch aborts here.
    registry::fetch_tarball(&resolved.url, &tarball)?;
