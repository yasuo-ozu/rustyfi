//! `space_between_math_kinds` (`math.ml:319-410`) as a TABLE.
//!
//! Every width below is an exact arithmetic identity under the `Mono` metrics
//! stub (advance = size/2 for every char, no OpenType MATH table, so
//! `MathC::script_scale` takes its 0.7 fallback and `superscript_kern`
//! contributes exactly zero) — so each one pins ONE row of the table and fails
//! if that row is dropped, reordered past an earlier row, or given the wrong
//! ratio. The ratios themselves are `primitives.cppo.ml:528-533`'s (`bin` 0.25,
//! `rel` 0.375, the other four 0.125).
...
/// `Length` is an `f64` newtype with no approximate comparison, and the engine
/// reaches each width by a different association of the same products than an
/// expectation written left-to-right does. One ULP of slack, four orders of
/// magnitude below the narrowest quantity these tests distinguish (a 1.05 pt
/// operator space).
#[track_caller]
...
/// `${a,b}` gets punctuation space AFTER the comma and none before it, because
/// `(Ord, Punct)` has no row and falls through to `_`.
#[test]
...
/// `(Ord, Op)` and `(Op, Ord)` both take `space_math_op`. `math-char MathOp` is
/// the only way to reach the class — no character in `default_math_class_map`
/// is an operator — so nothing but `\sum`/`\prod`/`\int`-style commands
/// exercises these rows.
#[test]
...
/// so `${a^{b+c}}`'s script sets three glyphs flush.
#[test]
