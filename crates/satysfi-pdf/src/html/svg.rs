...
use satysfi_backend::{Closing, Color, GraphicsElem, Path, PathSeg, PureHorzBox};
use std::fmt::Write as _;
use std::sync::atomic::{AtomicUsize, Ordering};

/// The nested-emitter callback type — the `&mut String` analogue of
/// `satysfi-pdf`'s `NestedEmitter` (`lib.rs:681`). [`emit_graphics`] invokes
...
mod tests {
    ...
    fn path_d_maps_line_and_bezier_segments() {
        let path = Path {
            subpaths: vec![satysfi_backend::Subpath {
