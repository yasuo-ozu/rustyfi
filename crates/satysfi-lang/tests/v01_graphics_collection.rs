...
use satysfi_backend::{Color, FontKey, FontMetrics, GraphicsElem, Length};
use satysfi_lang::ast::Ast;
use satysfi_lang::eval::Interp;
use satysfi_lang::value::{Env, Value};
use satysfi_lang::{prim_types, primitives};
...
fn try_call(
    ...
    name: &str,
    args: Vec<Value>,
) -> Result<Value, satysfi_lang::eval::EvalError> {
    let mut f = env
        .lookup(name)
        .unwrap_or_else(|| panic!("{name} is not bound"));
    for a in args {
...
fn inline_graphics_callback_result_coerces_per_version() {
    ...
    interp01.version = SatysfiVersion::V0_1;
    let env01 = primitives::base_env_with_version(SatysfiVersion::V0_1);
    ...
    match v {
        Value::InlineBoxes(boxes) => {
            ...
            match &boxes[0] {
                satysfi_backend::HorzBox::Pure(satysfi_backend::PureHorzBox::Graphics {
                    elems,
                    ..
    ...
    interp01b.version = SatysfiVersion::V0_1;
...
fn tabular_rules_callback_result_coerces_per_version() {
    ...
    match v {
        Value::InlineBoxes(boxes) => match &boxes[0] {
            satysfi_backend::HorzBox::Pure(satysfi_backend::PureHorzBox::Tabular(tab)) => {
...
fn deco_callback_result_coerces_per_version_when_fired() {
    use satysfi_backend::{DecoId, Page, PageGeometry, PlacedLine, PureHorzBox};
    use satysfi_lang::eval::DecoEntry;
    ...
    satysfi_lang::fire_hooks(&mut interp01, &doc01).expect("fire_hooks must succeed under V0_1");
    ...
    satysfi_lang::fire_hooks(&mut interp006, &doc006)
    ...
    let err = satysfi_lang::fire_hooks(&mut interp01b, &doc01b).unwrap_err();
    assert!(err.msg.contains("graphics"), "got: {}", err.msg);
}
