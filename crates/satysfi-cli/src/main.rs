...
fn cmd_compile(m: &ArgMatches) -> anyhow::Result<()> {
    ...
    // *before* the expensive compile+render, so a hit skips them entirely and
    // writes byte-for-byte the PDF an earlier miss rendered and stored.
    ...
    let cache_key = cache
        .as_ref()
        .and_then(|_| cache::compute_key(&program, env!("CARGO_PKG_VERSION"), version, &input));
