...
//! (`lib-satysfi/dist/fonts/ipaexm.ttf`, fetched by
...
use satysfi_pdf::{FontFlags, FontRegistry};
...
fn cjk_sentence_renders_real_glyphs_end_to_end() {
    ...
    // `FontRegistry::discover` finds `lib-satysfi/dist/hash/*.satysfi-hash`
    ...
    let registry = FontRegistry::discover(Some(&lib_root()), None, &FontFlags::default())
        ...
        .unwrap_or_else(|| {
            panic!(
                "lib-satysfi/dist/hash/fonts.satysfi-hash must be present after \
    ...
    let tmp = std::env::temp_dir().join(format!("satysfi-rust-cjk-{}.pdf", std::process::id()));
