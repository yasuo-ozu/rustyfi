//! `satysfi-envelope.yaml` decoder — Ld3b-1 (Axis B increment Ld3b, §3.2/§5.2
...
//! validates one `satysfi-envelope.yaml` file. The Ld3b-2 "reader" half
...
//! tests, including all 19 real upstream `satysfi-envelope.yaml.expected`
...
use satysfi_syntax::cst_v1::FileV1;
...
/// Decoded `satysfi-envelope.yaml` — `envelopeConfig.ml:118-139` /
...
/// Decode + validate one `satysfi-envelope.yaml`. ≈ `EnvelopeConfig.load`,
    ...
    let sources = match &config.contents {
        EnvelopeContents::Library {
            ...
            for path in source_paths {
                ...
                let file = satysfi_syntax::parse_file_v1(&src).map_err(|source| {
                    LoadError::Parse {
                        path: path.clone(),
                        source,
                    }
...
mod tests {
    ...
    const ANNOT: &str = include_str!(concat!(
        ...
        "/tests/fixtures/v01x/envelope/annot.satysfi-envelope.yaml"
    ));
    const MD_JA: &str = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/tests/fixtures/v01x/envelope/md-ja.satysfi-envelope.yaml"
    ));
    const FONT_LATIN_MODERN: &str = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/tests/fixtures/v01x/envelope/font-latin-modern.satysfi-envelope.yaml"
    ));
...
"
    ...
    fn envelope_file_missing_is_not_found() {
        let path =
            std::env::temp_dir().join("satysfi-loader-envelope-test-does-not-exist.yaml");
    ...
    fn envelope_load_config_reads_from_disk() {
        let path = std::env::temp_dir().join(format!(
            "satysfi-loader-envelope-test-load-config-{}-{}.yaml",
            std::process::id(),
    ...
    /// upstream checkout is present (env var `SATYSFI_UPSTREAM_GIT`),
    /// `git show`s and decodes all 19 real
    /// `satysfi-envelope.yaml.expected` files at
    ...
    fn envelope_sweep_all_19_upstream_fixtures_decode() {
        let Ok(upstream_git) = std::env::var("SATYSFI_UPSTREAM_GIT") else {
            eprintln!(
                "skipping envelope_sweep_all_19_upstream_fixtures_decode: \
                 SATYSFI_UPSTREAM_GIT is not set (see the Ld3b spec §10.1)"
            );
            return;
        };
        ...
        for pkg in PACKAGES {
            let rel = format!("lib-satysfi/packages/{pkg}/satysfi-envelope.yaml.expected");
