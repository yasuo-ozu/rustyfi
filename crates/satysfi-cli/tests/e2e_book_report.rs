...
/// The repo's `lib-satysfi/` directory, resolved the same way `e2e.rs`'s own
...
fn v01_stdjabook_capstone_renders_to_extractable_text() {
    ...
    run_with_big_stack(move || {
        ...
        let tmp = std::env::temp_dir()
            .join(format!("satysfi-rust-e2e-v01-stdja-book-{}.pdf", std::process::id()));
...
fn v01_stdjareport_capstone_renders_to_extractable_text() {
    ...
    run_with_big_stack(move || {
        ...
        let tmp = std::env::temp_dir()
            .join(format!("satysfi-rust-e2e-v01-stdja-report-{}.pdf", std::process::id()));
        std::fs::write(&tmp, &bytes).unwrap();
        let pdftotext = Command::new("pdftotext").arg(&tmp).arg("-").output();
        match pdftotext {
            Ok(out) if out.status.success() => {
