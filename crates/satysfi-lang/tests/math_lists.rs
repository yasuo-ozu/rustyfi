...
//    `satysfi-syntax`'s own `roundtrip.rs`) — proves this pipeline's parser
//    accepts the same source `assert_roundtrip` covers.
// ============================================================================

#[test]
fn math_list_sources_parse() {
    satysfi_syntax::parse_file("${| a | b |}").expect("${| a | b |} should parse");
    satysfi_syntax::parse_file("${|}").expect("${|} should parse");
}
