...
//! `write_named_dests`/`write_outline` (`satysfi-pdf/src/lib.rs`), driven
//! through the public `render_pdf_with` entry point with a hand-built
//! `DocExtras` (no lang layer involved — see `crates/satysfi-cli/tests/
...
fn render_pdf_wrapper_is_byte_identical_to_render_pdf_with_default_extras() {
    let geometry = geometry();
    let pages = vec![Page::default()];
    let a = satysfi_pdf::render_pdf(&geometry, &pages, &[]).expect("render_pdf");
    let b = satysfi_pdf::render_pdf_with(&geometry, &pages, &[], &DocExtras::default())
...
fn empty_extras_emit_no_annots_dests_or_outlines_catalog_keys() {
    let geometry = geometry();
    let pages = vec![Page::default()];
    let bytes = satysfi_pdf::render_pdf_with(&geometry, &pages, &[], &DocExtras::default())
        .expect("render");
    let hay = String::from_utf8_lossy(&bytes);
