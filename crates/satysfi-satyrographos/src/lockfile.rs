...
impl Lockfile {
    ...
    /// (`satysfi-cli/src/cache.rs::compute_key`): a re-solve that changes any
    ...
    /// entry (the pin `reconcile`/`solve` chose) or `source.path` for a path
    /// entry (there is no version, but the path participates so a
    /// re-pointed — same-name, same-hash-coincidentally — path entry still
    /// changes the digest); a `git`/other entry falls back to `""` (parsed
    /// but never materialised through phase 3, `Error::UnsupportedSource`).
