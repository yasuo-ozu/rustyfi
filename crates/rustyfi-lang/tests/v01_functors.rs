//! End-to-end integration tests for first-order
//! single-parameter functors, driven through the REAL public pipeline
//! (`rustyfi_lang::compile_document_v1`) — the same harness
//! `tests/v01_sealing.rs` uses; see that file's doc comment for the
//! `NotADocument` trick. Because evaluation genuinely runs to discover the
//! result isn't a `Value::Document`, `NotADocument` also pins that the
//! functor machinery produces a program that both type-checks AND
//! evaluates, not merely one that elaborates.
...
/// I-fn1: `map.satyg`'s `include Make Int` shape — a functor over a tiny
/// `Ord`-like parameter, applied to a concrete module, its result
...
/// I-fn2: `code.satyh`'s `module Default = Make DefaultSettings` shape — a
/// functor over a value-only NAMED signature parameter, referenced
/// QUALIFIED (`Lib.Default.…`), simplified to a plain `val` rather than a
/// real inline command.
#[test]
...
";
    ...
    // Two applications are independently instantiated
    // — `Console` reads ITS OWN argument, not `Default`'s.
    assert_accepts(lib, "Lib.Console.get-label 0");
}

/// I-fn3: I-fn1's shape, but the argument is missing `compare` — a
/// precise, functor-framed compile error, end-to-end.
#[test]
...
/// A functor DEFINITION alone (never applied) compiles with zero residue
/// from the functor itself — the erasure invariant.
#[test]
...
// A functor BODY that APPLIES another functor (a
// parameter flowing as a functor ARGUMENT) plus relative-sibling-head
// absolutization (a reference whose head names a SIBLING nested module) —
// `set.satyg`'s and `code.satyh`'s shapes.

/// F-set1 (end-to-end): `Set.Make`'s body applies `Map.Make` to ITS OWN
/// parameter (`module Impl = Map.Make Elem`), then references the result
/// through RELATIVE sibling paths (`Impl.t`/`Impl.wrap`/`Impl.cmp`) — both
/// halves of 2f-2a in one fixture. Generativity (2f-2b's `F-gen`) is NOT
/// exercised here; this pins only that the shape compiles and evaluates.
#[test]
...
/// F-set1-neg: the same shape, but the inner application's argument
/// (`Elem`) does not satisfy `Map.Make`'s declared parameter signature — a
/// precise, functor-framed compile error surfaces through the app-in-body
/// path too.
#[test]
...
/// F-abs2: a PLAIN (non-functor) module whose body references a SIBLING
/// nested module by a relative dotted path — retires the 2d-3 "relative
/// sibling module references" gap for non-functor expressions too.
#[test]
...
/// dotted path (`+Inner.greet`), absolutized the same way as value/type
/// sites, no guard (unlike the functor-parameter case).
#[test]
