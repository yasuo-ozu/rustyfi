...
fn a_frames_deco_underlay_draws_before_the_pages_first_bt() {
    ...
    let bytes = rustyfi_pdf::render_pdf_with(&geometry, std::slice::from_ref(&page), &[], &extras)
...
fn an_empty_overlay_emits_no_graphics_state_wrapper_at_all() {
    let geometry = geometry();
    let page = page_with_text();
    let with_default = rustyfi_pdf::render_pdf_with(
        &geometry,
    ...
    .expect("render");
    let without_extras =
        rustyfi_pdf::render_pdf(&geometry, std::slice::from_ref(&page), &[]).expect("render");
