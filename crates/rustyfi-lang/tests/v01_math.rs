...
use rustyfi_lang::{prim_types, primitives, CompileError};
...
fn v01_mini_file() -> LoadedFile {
    let path = repo("lib-rustyfi/dist-v01/packages/v01-mini.satyh");
    let src = std::fs::read_to_string(&path).expect("read v01-mini.satyh");
    LoadedFile {
        path,
        cst: LoadedCst::V0_1(
            rustyfi_syntax::parse_file_v1(&src).unwrap_or_else(|e| panic!("v01-mini.satyh: {e}")),
        ),
...
fn entry_file_from_disk(rel: &str) -> LoadedFile {
    ...
    LoadedFile {
        path,
        cst: LoadedCst::V0_1(
            rustyfi_syntax::parse_file_v1(&src).unwrap_or_else(|e| panic!("{rel}: {e}")),
        ),
        origin: Default::default(),
        version: RustyfiVersion::V0_1,
    }
...
fn entry_file_inline(src: &str) -> LoadedFile {
    LoadedFile {
        ...
        cst: LoadedCst::V0_1(
            rustyfi_syntax::parse_file_v1(src).unwrap_or_else(|e| panic!("inline entry: {e}")),
        ),
        origin: Default::default(),
        version: RustyfiVersion::V0_1,
    }
...
fn lib_file_inline(name: &str, src: &str) -> LoadedFile {
    LoadedFile {
        ...
        cst: LoadedCst::V0_1(
            rustyfi_syntax::parse_file_v1(src).unwrap_or_else(|e| panic!("inline lib {name}: {e}")),
        ),
        origin: Default::default(),
        version: RustyfiVersion::V0_1,
    }
...
"#;
    ...
    let mono = Mono;
    let err = rustyfi_lang::compile_document_v1_with_trials(&files, &mono)
...
fn here_constant_is_unbound_in_006() {
    ...
    assert!(
        prim_types::primitive_type_with_version("here", RustyfiVersion::V0_1).is_some(),
...
fn eval_v01_raw_value(
...
) -> Result<rustyfi_lang::value::Value, String> {
    ...
    let body = rustyfi_lang::v1::lower::lower_document_v1(entry_cst)
    ...
    rustyfi_lang::typecheck::typecheck_verbose_with_version(&program, RustyfiVersion::V0_1)
...
fn t_m2_paren_closure_route_draws_fill_ink() {
    use rustyfi_backend::{GraphicsElem, HorzBox, PureHorzBox};
...
";
    ...
    let rustyfi_lang::value::Value::Tuple(parts) = v else {
        panic!("expected a 3-tuple");
    };
    assert_eq!(parts.len(), 3);
    fn first_cp(v: &rustyfi_lang::value::Value) -> i64 {
        match v {
            rustyfi_lang::value::Value::List(cps) => match &cps[0] {
                rustyfi_lang::value::Value::Int(n) => *n,
