...
fn eval_str(src: &str) -> Value {
    ...
    interp.eval(&env, &program.body).expect("eval")
}
...
fn embed_block_bottom_wraps_the_solidified_block_at_the_given_width() {
    ...
    match v {
        Value::InlineBoxes(boxes) => {
            ...
            match &boxes[0] {
                satysfi_backend::HorzBox::Pure(PureHorzBox::EmbeddedBlock {
                    width,
                    height,
                    depth,
                    block,
...
fn line_stack_bottom_stacks_each_element_as_one_line_at_the_widest_natural_width() {
    ...
    match v {
        Value::InlineBoxes(boxes) => {
            assert_eq!(boxes.len(), 1);
            match &boxes[0] {
                satysfi_backend::HorzBox::Pure(PureHorzBox::EmbeddedBlock { width, block, .. }) => {
