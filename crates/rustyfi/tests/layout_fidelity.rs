...
//! `pdf_image_diff`) because it needs poppler, python3, and the vendored
//! corpus; the script itself self-skips (exit 0) when a prerequisite is
...
fn layout_matches_upstream_satysfi_within_baseline() {
    ...
    // silently becomes "port vs whatever this box has". Measured: with the
    // flake's `satysfi` merely on PATH, enumitem was reported as a regression
    // (text_match 0.8516 against a 0.8891 baseline, width_p95 1.79pt against
    // 0.74pt) while the port's own output was byte-for-byte what the baseline
    // was recorded from — its word count, 2225, matched exactly; it was
    // upstream's that had moved, 2285 -> 2289.
    //
    // So regenerating is opt-in, for the one job it is for: deliberately
    // refreshing the references, which must be followed by
