...
use satysfi_loader::LoadedProgram;
...
impl Cache {
    ...
    /// 2. `$XDG_CACHE_HOME/satysfi-rust`;
    /// 3. `$HOME/.cache/satysfi-rust`;
    /// 4. a `satysfi-rust-cache` directory under the system temp dir.
    ///
...
fn resolve_dir(override_dir: Option<PathBuf>) -> PathBuf {
    ...
    if let Some(xdg) = non_empty_env("XDG_CACHE_HOME") {
        return PathBuf::from(xdg).join("satysfi-rust");
    }
    if let Some(home) = non_empty_env("HOME") {
        return PathBuf::from(home).join(".cache").join("satysfi-rust");
    }
    std::env::temp_dir().join("satysfi-rust-cache")
}

/// `$VAR` if it is set and non-empty, else `None` (an empty `XDG_CACHE_HOME`
/// must not resolve to a bare `/satysfi-rust`).
